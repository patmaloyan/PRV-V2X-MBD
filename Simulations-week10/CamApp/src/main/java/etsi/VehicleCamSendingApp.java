...
public class VehicleCamSendingApp extends AbstractCamSendingApp<VehicleOperatingSystem> {
    ...
    SensorErrorModel sensorErrorModel;
    ...
    private boolean windowSamplingStarted = false;
    private boolean radioAndPerceptionEnabled = false;

    @Override
    public void onStartup() {
        getLog().debugSimTime(this, "Initialize application");
        sensorErrorModel = new SensorErrorModel(getRandom());
        data.alias = nextAlias();
        
        if (getConfiguration().enableDriverProfiles) {
            double randomValue = getRandom().nextDouble();
        ...
        long communicationStart = getConfiguration().simulationTime.start * 1_000_000_000L;
        if (getOperatingSystem().getSimulationTime() >= communicationStart) {
            startWindowSampling(null);
        } else {
            getOperatingSystem().getEventManager().addEvent(communicationStart, this::startWindowSampling);
        }
    }

    private void startWindowSampling(Event event) {
        if (!canProcessEvent() || windowSamplingStarted) {
            return;
        }
        long communicationEnd = getConfiguration().simulationTime.end * 1_000_000_000L;
        if (getOperatingSystem().getSimulationTime() >= communicationEnd) {
            return;
        }
        windowSamplingStarted = true;
        scheduleNextPseudonymChange();
        firstSample();
    }

    private void ensureRadioAndPerception() {
        if (radioAndPerceptionEnabled) {
            return;
        }
        ...
        radioAndPerceptionEnabled = true;
        getLog().infoSimTime(
                this,
                "Communication and perception enabled in-window with wall occlusion, angle {} deg and range {} m",
                VIEWING_ANGLE,
                VIEWING_RANGE
        );
    }
    ...
    private long nextAlias() {
        return getRandom().nextLong(1_000_000_000L, 10_000_000_000L);
    }
